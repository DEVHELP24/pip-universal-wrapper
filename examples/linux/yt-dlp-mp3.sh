...
for item in "${items[@]}"; do
   which yt-dlp > /dev/null && yt-dlp --ignore-errors --format bestaudio \
        --extract-audio --audio-format mp3 \
        --output '%(title)s.%(ext)s' --paths "$OUTDIR" "$item" || python3 pip-universal-wrapper.py --exec yt-dlp --ignore-errors --format bestaudio \
        --extract-audio --audio-format mp3 \
